...
export class CountryServiceService {
  ...
  private fetchAllPages(): Observable<any[]> {
    ...
    const page1 = this.http.get<any>(`${this.BASE_URL}?response_fields=${fields}&limit=${limit}&offset=0&api-key=${this.API_KEY}`);
    const page2 = this.http.get<any>(`${this.BASE_URL}?response_fields=${fields}&limit=${limit}&offset=100&api-key=${this.API_KEY}`);
    const page3 = this.http.get<any>(`${this.BASE_URL}?response_fields=${fields}&limit=${limit}&offset=200&api-key=${this.API_KEY}`);
  ...
  getCountryByCode(code: string): Observable<any> {
    ...
    const url = `${this.BASE_URL}/codes.alpha_2/${code}?response_fields=${fields}&api-key=${this.API_KEY}`;
    return this.http.get<any>(url).pipe(
      map(response => response.data.objects[0]),
      tap(detail => this.countryDetailCache.set(code, detail)) // cache after first fetch
    );
  }
}